...
Log.prototype._getEvent = function(id, noContent) {
  let r = this.r();
  let q = r.table(this.eventTable).get(id);
...
Log.prototype.query = function(opts = {}, orderBy = 'received') {
  ...
  let filter = FILTERS.find(filter => opts[filter]) || 'gt';
  let ts = opts[filter] || Date.now();

  let q = r.table(this.eventTable).orderBy({ index: r.desc(orderBy) });

  if (opts.types && opts.types !== '*') {
    q = q.filter(row => r.and(
      r.expr(opts.logs).contains( row('log') ),
      r.expr(opts.types).contains( row('type') ),
      row(orderBy)[filter](ts)
    ));
  } else {
    q = q.filter(row => r.and(
      r.expr(opts.logs).contains( row('log') ),
...
Log.prototype.addToMany = function(logs, event, opts = {}) {
  ...
  let events = logs.map(log => Object.assign({
...
Log.prototype.addToManyLinked = function(logs, event, content) {
  ...
  return this.db.table(this.contentTable)
    ...
    .run()
    .then(res => {
      let cid = content.id || res.generated_keys[0];
      let events = logs.map(log => Object.assign({
        status: STATUS.CREATED
      }, event, {
        log,
...
Log.prototype.add = function(event, opts = {}) {
  ...
  if (opts.linkedContent && typeof event.content === 'object') {
    return this.addLinked(event);
  }

  return this._add(event);
};

Log.prototype.addLinked = function(event) {
...
Log.prototype.setContent = function(id, content, returnChanges) {
  ...
  return r.table(this.eventTable)
    ...
    .then(cid => {
      if (cid) {
        let q = r.table(this.contentTable).get(cid);
